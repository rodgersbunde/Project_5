                ...
                if recipe_name_search:
                    with st.spinner('Searching for recipe...'):
                        ...
                        if not filtered_recipes.empty:
                            ...
                            ai_recipe = get_chatgpt_recipe(recipe_name_search)
                            if ai_recipe:
                                with st.container():
                                    st.markdown('<div class="recipe-card">', unsafe_allow_html=True)
                                    st.subheader(f"🤖 AI-Generated Recipe: {recipe_name_search}")
                                    st.markdown(f"**Ingredients:**\n{ai_recipe['ingredients']}")
                                    st.markdown(f"**Instructions:**\n{ai_recipe['instructions']}")
                                    st.markdown('</div>', unsafe_allow_html=True)
                            else:
                                st.error("Could not generate recipe suggestion. Please try again later.")
